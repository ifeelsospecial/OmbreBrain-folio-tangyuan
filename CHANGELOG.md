...
## 2026-09-26 · 「关于你」偏好档案（取上游 3.5.0 You 的核心、做轻）

- 起因：同样让推荐罗马景点，ChatGPT 更合口味——它有一份常驻的短偏好档案，这里只有 600 多条事件记忆，偏好散在里面没人提炼。
- `about.py`：按方面（旅行节奏 / 爱看什么 / 吃的口味 / 雷区 / 低落时需要什么 / 说话习惯 / 生活习惯）的一句话条目，存在 `profile/about.md`（不进普通记忆列表，跟着 buckets 进备份）。他新记的是「观察中」，在 2 个不同日子看到才定下；她在页面上点「对」当场定下；从记忆提炼出的条目不计天数，重跑提炼不会替她定下来。
- 开场注入：breath 与 breath-hook 最前面带上已确定部分（总长 ≤1200 字），观察中的只报个数。
- MCP 工具 `you`（记 / 确认 / 改 / 删 / 读）；`GET/POST /api/about`；`POST /api/about/draft` 用 AI 从现有记忆分批提炼第一版（全部观察中，等她审）。
- 页面 `/v2/about/`：待确认卡片（对 / 改一下 / 不对）+ 七个方面的卡片，随手加一句、改、删；空档案时一键「让他从记忆里整理一版」并显示进度。各处导航、手机设置、小岛菜单都有入口。
- 合并进已有记忆的新内容也会认地点（并进原有地点，不覆盖），网页端存的"今天去了泰特"并进"伦敦旅行"时不再漏掉。
- 测试：新增 `tests/test_about.py`（5 例）；已验证把定下所需天数改成 1、去掉"提炼不计天数"时对应用例失败。全量 175 passed。

## 2026-09-26 · 知识本 + 三个页面重新设计 + 小岛入口

- **知识本规则**：带 `handbook` / `知识` 标签的记忆是知识——不衰减、不归档，不进日常浮现、永久参考区、做梦、每周回顾与"那天"，但检索照常能找到。线上已有 36 条（31 条原本会慢慢淡掉；4 条钉选的仍按钉选显示）。
- `breath_advanced(domain="知识")` 按主题列出全部知识；`GET/POST /api/notes`。
- 认出的城市并进标签：检索"伦敦"能命中在伦敦的记忆，星图里同城记忆相连。地图区分"在这里学到的"。
- 回顾页新增撤销（`action=undo`，只撤回顾页自己写下的标记）。
- **页面重新设计**（`v2/island-ui.css` / `island-ui.js` 共用）：纸页质感、Cormorant Garamond + 思源宋体、两套皮肤（跟随 OB 主题；从小岛打开 `?skin=island` 用小岛雾蓝）与深色模式。
  - 足迹：纸色底图、按首次到访串起的旅程虚线、带次数的城市圆点（最近一城有呼吸光）、旅程时间线侧栏、城市详情分"学到的 / 记忆"；手机端紧凑标题卡、缩远时只标最大的三座城。
  - 知识本 `/v2/notes/`：杂志式瀑布流、主题色、搜索高亮、阅读层、"记一条"表单。
  - 回顾：一次一张卡的牌堆、大号日期、放下下沉 / 留着上浮动画、左右滑动与方向键、撤销、进度与小结。
- 测试：新增 4 例（知识不浮现但可检索与通道、notes 接口、城市并入标签、衰减不归档知识）。页面经无头 Chrome 实测：桌面/手机 × 两套皮肤 × 深色，无溢出无脚本错误；地图点城市、知识本读与写、回顾放下与撤销流程都跑通。全量 170 passed, 7 skipped。

## 2026-09-26 · 为这个库定制的五个功能

依据线上 628 条记忆的统计（只看元数据分布，不逐条读内容）挑的：

- **capture-hook 存原文**：线上没有一条记忆带 `raw_source`，`source` 工具永远查不到原话。capture-hook 现在把 our-island 发来的那一来一回原文挂到这次拆出/合并到的每条记忆上（合并时追加，上限 8000 字保留最新部分）。grow 的 MCP 参数不变（内部拆出 `_grow_impl`）。dashboard 的"按原文重新提炼正文"也随之可用。
- **一年前 / 一个月前的今天**：breath 与 breath-hook 开场各附最多一条（事件日优先，同日挑重要度×情绪强度最高的；已沉底的记忆在"那天"仍会被想起；已在普通浮现里出现的不重复）。`surfacing.on_this_day` 可关。
- **约定带日期**：`plan(due=YYYY-MM-DD)`、`trace(due=...)`（仅 plan；空串去掉日期）。进行中的约定 3 天内到期或过期不超过 7 天时，开场最前面单独列出。
- **每周回顾** `/v2/review/`：只有 3/578 条动态记忆被放下过，浮现池一直是同一批旧事。列出两周前发生、两周没再被想起、重要度 ≤ 6、不是导入或亲手写的普通记忆，最不重要的排前面；"放下"= resolved（不刷新激活时间），"留着"= 60 天内不再问。`GET /api/review/candidates`、`POST /api/review/decide`。桌面导航与手机设置都加了入口；周日且待回顾 ≥ 5 条时 breath-hook 提醒一句。
- **足迹地图** `/v2/map/`：新存的出行类记忆（领域含"出行"或有明确去向/场所字眼）在后台用 AI 认地点与近似坐标，写进 `places`；`GET /api/places` 按城市聚合（含归档区）；页面用本地托管的 Leaflet 1.9.4 + OpenStreetMap 底图，深色模式滤镜压暗。存量用 `POST /api/places/backfill` 或 workflow「给存量记忆认地点」（看过的不再看）。
- **重新分类「未分类」**：`POST /api/reclassify/uncategorized` + workflow「重新分类「未分类」记忆」，用服务端当前 AI 配置，只改 domain/tags（合并），不动名字、情绪、重要度与激活时间，并把文件挪进对应领域目录。取代只能在 Docker 里跑的 `reclassify_api.py`。
- 新增 `BucketManager.rewrite_bucket_metadata()` 的更多用处；`v2/theme-system.compiled.js`（由 `v2/mobile/build.mjs` 顺带生成）供不走浏览器内编译的独立页面使用。
- 测试：新增 `tests/test_tailored.py`（14 例）。页面经无头 Chrome 在手机与桌面视口实测：无横向溢出、无脚本错误，地图点城市可列出记忆。全量 166 passed, 7 skipped。

## 2026-09-26 · 关系门槛按本仓数据校准

- 用上游门槛（0.72 / 0.75 / 0.85）回填线上 585 条记忆后：中位每条 7 条关系、228 条（约 40%）撞满每桶 8 条上限、593 条单向边（上限截断造成）。上游在自己的语料上校准时是中位 2 条、3.5% 撞上限——本仓记忆集中在同一个人、同一段生活，相似度整体偏高。
- 三档门槛同时平移 +0.08：相关 0.80、前段/后续 0.83（72 小时内）、同一事件 0.93（6 小时内）。按回填里存下的分数估算，相关 0.80 时中位 1 条、约 5% 撞上限、361/585 条至少有一条关系。可由 `config.relations` 覆盖；上游规则文件未改动，门槛判断在 `relation_link.infer_type()`。
- 回填新增 `rebuild`：`POST /api/relations/backfill?rebuild=1` / workflow 勾选 rebuild，先清掉所有自动关系（含归档区；手动改过的保留），再按当前门槛重建。新增 `BucketManager.rewrite_bucket_metadata()`。
- 测试：新增 2 例（校准默认值与配置覆盖、重建只清自动关系）；已验证重建不清时失败。全量 152 passed, 7 skipped。

## 2026-09-26 · 关系存量回填 + 记忆星图显示关系

- **存量回填**（本 fork 新增）：`POST /api/relations/backfill` 在后台给存量记忆补建自动关系，`GET` 查进度；仅管理员通道。按 (created, id) 全序从旧到新遍历，每条只和排在它之前的桶连，每一对恰好处理一次、方向与实时写入一致；复用同一套阈值与 `merge_auto_links`，已有关系（含手动改过的）不改写，可重复运行。每条一次 embedding 查询，条间停 0.2 秒防限流。
- 新增手动触发的 workflow「补建存量记忆的桶间关系」，用每日备份同一组 `OMBRE_BACKUP_URL` / `OMBRE_ADMIN_TOKEN` 调用并跟踪进度。
- **记忆星图**：`/api/buckets` 每条带 `relations`（目标、类型、中文标签）；星图把关系画成强调色虚线，不参与连线数上限截断；和共享标签的连线重合时合并成一条关系线。右侧"关联记忆"里关系显示为"关系 · 后续 / 前段 / 相关…"，按当前这条记忆自己的视角取标签。
- 测试：`tests/test_relations.py` 新增 2 例（回填方向 / 重跑不重复 / 同秒创建也能连上；接口进度与列表字段）。全量 150 passed, 7 skipped。前端改动经 Babel 编译检查，本地用备份副本验证了列表接口输出。
