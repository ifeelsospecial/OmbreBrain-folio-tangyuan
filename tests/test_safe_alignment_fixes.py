...
async def test_tool_parameters_preserve_explicit_zero_and_feel_tags(tmp_path, monkeypatch):
    ...
    # 本 fork: breath 函数保留旧签名(兼容缓存旧 schema 的客户端), 但对外公布的 MCP schema 必须是 0 参数
    breath_tool = server.mcp._tool_manager.get_tool("breath")
    assert breath_tool.parameters.get("properties") == {}
