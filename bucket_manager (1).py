...
# 3 = 全量视野(默认: admin-token 通道, dashboard, 后台任务) → 见所有 level
# 2 = 中间视野(claude.ai 的 URL-key 通道) → 只见 level<=2 的桶
# 1 = (预留)
# server.py 的鉴权中间件按进门方式 set 这个值。
# ============================================================
ACCESS_LEVEL = contextvars.ContextVar("ombre_access_level", default=3)
...
class BucketManager:
    ...
    async def create(
        ...
        metadata = {
            ...
            "activation_count": 0,
            # 记忆分级: 1=通用, 2=亲密但可出版(官网+API可见), 3=仅API可见。
            # 旧桶无此字段, 读取方用 meta.get("level", 1) 兜底 → 天然向后兼容。
            # 写入者必须永远能读回自己写的东西 → level 被钳到 min(传入值, 当前通道ACCESS_LEVEL)。
            "level": min(int(level), ACCESS_LEVEL.get()) if int(level) in (1, 2, 3) else 1,
        }
