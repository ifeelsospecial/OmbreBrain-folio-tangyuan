...
OAuth 只给 `/mcp` 记忆读写能力，不给管理 API。本 fork 中 OAuth 通道与 URL-key 通道同为受限视野（level 1），看不到 level-2 私密记忆。授权码强制 PKCE S256，访问令牌一小时过期，刷新令牌使用一次就轮换；磁盘只保存令牌哈希。
...
1. 设 `OMBRE_ENABLE_SELF_UPDATE=1`；本 fork 默认更新源是 `ifeelsospecial/OmbreBrain-folio-tangyuan`（自己的仓库）。**不要**把 `OMBRE_UPDATE_REPO` 指向 folio 或上游，否则会覆盖本 fork 的私密分级与 recall/capture hook。部署在 Render 上时不需要这个功能：推送 main 即自动部署。
